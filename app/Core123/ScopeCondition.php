...
trait ScopeCondition
{
    ...
    public function scopeWhere($result, $where, $prefix = '')
    {
        $result = $result->where(function ($q) use ($where, $prefix)
        {
            foreach ($where as $item)
            {
                list($col, $ope, $val) = $item;

                if($prefix && !in_array($col, ['id', 'created_at', 'updated_at', 'approve_at']))
                {
                    $col =  $prefix.'_'.$col;
                }
    ...
    public function scopeMultiOrder($result, $order = [], $prefix = '')
    {
