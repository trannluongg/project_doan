...
class PermissionEditRequest extends RequestAbstract
{
    protected $permissionRepository;

    public function __construct(PermissionRepositoryInterface $permissionRepository, array $query = [], array $request = [], array $attributes = [], array $cookies = [], array $files = [], array $server = [], $content = null)
    {
    ...
    {
        $permission = $this->permissionRepository->firstById($this->id);

        return [
            'name'                  => 'required|string|max:50|min:2|unique:permissions,name,'. $permission->id,
            'identifier_name'       => 'required|string|max:50|min:2|unique:permissions,identifier_name,'. $permission->id,
